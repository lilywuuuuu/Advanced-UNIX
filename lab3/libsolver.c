...
int maze_init() {
    ...
    void *main_addr = maze_get_ptr();
    printf("SOLVER: _main = %p\n", main_addr);

    // retrieve function pointers
    maze_init_func init_func = (maze_init_func)(uintptr_t)dlsym(handle, "maze_init");
    maze_load_func load_func = (maze_load_func)(uintptr_t)dlsym(handle, "maze_load");
    move_func move_up_func = (move_func)(uintptr_t)dlsym(handle, "move_up");
    move_func move_down_func = (move_func)(uintptr_t)dlsym(handle, "move_down");
    move_func move_left_func = (move_func)(uintptr_t)dlsym(handle, "move_left");
    move_func move_right_func = (move_func)(uintptr_t)dlsym(handle, "move_right");
    maze_free_func free_func = (maze_free_func)(uintptr_t)dlsym(handle, "maze_free");

    // load the maze from file
    maze = (*load_func)("./maze.txt");
    if (!maze) {
        fprintf(stderr, "Error loading maze.\n");
        dlclose(handle);
        return 1;
    }

    // solve the maze
    solveMaze(maze->cy, maze->cx, 0);
    for (int i = 0; i < maze->h; i++) {
        for (int j = 0; j < maze->w; j++) {
            if (visited[i][j]) {
                printf(". ");
            } else {
                printf("%c ", maze->blk[i][j] ? '#' : ' ');
            }
        }
        printf("\n");
    }

    // overwrite the function pointers
    for (int i = 0; i < path_size; i++) {
        unsigned long *addr = (unsigned long *)((unsigned long)main_addr - main_offset + move_offset[i]);
        if (make_memory_writable(addr, sizeof(unsigned long)) == -1) {
            printf("make_memory_writable failed\n");
            return 1;
        }
        if (path[i] == 'R') {
            *(unsigned long *)addr = (unsigned long)move_right_func;
        } else if (path[i] == 'D') {
            *(unsigned long *)addr = (unsigned long)move_down_func;
        } else if (path[i] == 'L') {
            *(unsigned long *)addr = (unsigned long)move_left_func;
        } else if (path[i] == 'U') {
            *(unsigned long *)addr = (unsigned long)move_up_func;
        }
    }

    // free the maze and close the shared library
    (*free_func)(maze);
    dlclose(handle);
